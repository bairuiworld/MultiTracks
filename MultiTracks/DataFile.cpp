...
bool CDataFile::Load(t_Str szFileName)
{
	...
	std::fstream File(szFileName.c_str(), std::ios::in|std::ios::_Nocreate);
...
t_Str CDataFile::GetValue(t_Str szKey, t_Str szSection, t_Str def) 
{
	t_Key* pKey = GetKey(szKey, szSection);
...
// Returns the key value as a t_Str object. A return value of
// t_Str("") indicates that the key could not be found.
t_Str CDataFile::GetString(t_Str szKey, t_Str szSection, t_Str def)
{
	return GetValue(szKey, szSection, def);
}

// GetFloat
// Returns the key value as a float type. Returns FLT_MIN if the key is
// not found.
float CDataFile::GetFloat(t_Str szKey, t_Str szSection, float def)
{
...
// not found.
int	CDataFile::GetInt(t_Str szKey, t_Str szSection, int def)
{
	t_Str szValue = GetValue(szKey, szSection);
...
// not found.
bool CDataFile::GetBool(t_Str szKey, t_Str szSection, bool def)
{
	bool bValue = false;
	t_Str szValue = GetValue(szKey, szSection, "_not_found");
